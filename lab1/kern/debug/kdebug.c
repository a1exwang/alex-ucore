...
print_stackframe(void) {
	...
	for (i = 0; i < STACKFRAME_DEPTH; ++i) {
		cprintf("fp: 0x%0x, pc: 0x%0x\nargs: ", current_fp, current_pc);
		for (j = 0; j < argc; ++j) {
			cprintf("0x%0x ", current_args[j]);
		}
		cprintf("\n---------------------------------------\n");
		print_debuginfo(current_pc);
		current_pc = *((uint32_t*) (current_fp + 4));
		current_fp = *((uint32_t*) current_fp);
		current_args = (uint32_t*)(current_fp + 8);
	}
}
